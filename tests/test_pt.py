...
class PytorchTests(AllTests, unittest.TestCase):
    ...
    def basic_momentum(self, x):
        ...
        layer = gala.MomentumNormalization(x.shape[-1], momentum=0.1)
        layer.train()
        ...
        npt.assert_allclose(mean(output.numpy()), 0.0, rtol=1e-2, atol=1e-2)
        npt.assert_allclose(std(output.numpy()), 1.0, rtol=1e-2, atol=1e-2)
